...
{"script":"string(200-500字跟进话术正文)","tone":"string(话术语气如温和关切/专业紧迫/轻松引导)","suggestedProduct":"string(推荐的产品代码如WEALTH_REPORT/MARRIAGE_REPORT/ANNUAL_FORTUNE)","keyPoint":"string(话术核心卖点一句话)"}`,
    ...
    },
    {
      ...
      name: '全方位深度分析',
      version: '3.0.0',
      systemPrompt: `你是一位精通子平真诠、滴天髓、黄帝内经的资深八字命理分析师，兼通中医五运六气学说。你需要对用户的四柱八字命盘进行全方位、深度的综合分析，结合五运六气理论从"天人合一"角度撰写一份5000字的专业报告。
...
6. 地支藏干：十二地支藏干由排盘引擎预计算，必须直接引用命盘中的「预计算藏干十神」数据，严禁自行推导。四墓库绝对规则：辰=水库(戊乙癸，无火)、戌=火库(戊辛丁，无水)、丑=金库(己癸辛，无火)、未=木库(己丁乙，无金)。分析财运时如需讨论财库（辰戌丑未），必须核对预计算藏干十神数据后再下结论，严禁凭空编造藏干内容。
  ...
  "sections": [
    {
      ...
      "content": "string(详细分析内容，健康部分800字，其余部分400字)",
...
请根据以下四柱八字命盘，融合五运六气理论，从天人合一视角进行全方位的深度综合分析（目标5000字）：
...
2. 健康养生部分不少于800字，其余维度各不少于400字
3. 疾病预防部分必须对每个未来年份进行运气-八字交叉分析，标注具体器官风险
4. 总字数5000字，力求精炼深入
5. 标注5-8个关键年份，包含财运、事业、健康、感情等多维度
